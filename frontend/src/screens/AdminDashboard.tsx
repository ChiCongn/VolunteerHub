...
import { Users, Calendar, TrendingUp, Download, Lock, Unlock, CheckCircle, XCircle } from 'lucide-react';
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button onClick={handleExport} className="gap-2 bg-[#43A047] hover:bg-[#388E3C]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-card border border-border rounded-lg p-6 space-y-2">
          ...
          <p className="text-3xl font-semibold">{events.filter(e => e.status === 'upcoming').length}</p>
            ...
            <Table>
              ...
              <TableBody>
                {events.filter(e => e.status === 'pending').map((event) => {
                  const organizer = users.find(u => u.id === event.managerId);
                  return (
          ...
          <div className="bg-card border border-border rounded-lg p-6">
            <div className="space-y-3">
              {[
                ...
                { action: 'Event approved', user: 'Admin', time: '5 hours ago' },
                { action: 'New event created', user: 'Michael Chen', time: '1 day ago' },
                { action: 'User updated profile', user: 'David Kim', time: '2 days ago' },
              ].map((log, index) => (
                <div
                  key={index}
